...
class Plan extends Admin {
	...
	public function watch() {
		...
		$ress = db('plan')->order('Id desc')->find(); //查询最新的一条
		$this->assign(['plan' => $res, 'analyze' => $ress]);
		return $this->fetch();
	...
	public function addplan() {
		...
		if (!empty($result)) {
			...
			$res = db('plan')->insert($value); //奖判断结果插入数据表
		}
		return $this->redirect(url('admin/plan/watch'));
	}

	//删除计划  根据id删除计划如果成功返回当前页,失败则返回删除失败
	public function deleteplan($id) {
		$res = db('plan')->delete($id);
		if ($res) {
			return $this->redirect(url('admin/plan/watch'));
		} else {
	...
	public function updateplan(Request $request) {
		...
		$str = $data['period']; //计划期间
		$strint = preg_replace('/\D/s', '', $str); //计划期间取数字
		$abort = str_pad($data['abort'], 3, "0", STR_PAD_LEFT); //预测期数三位,前面补零
		// echo $data['abort'];
		// echo "<br>";
		// echo $abort;
		...
		if ($value) {
			$res = db('plan')->where('id', $id)->update($value);
			return $this->redirect(url('admin/plan/watch'));
		} else {
			return '0';
		}
